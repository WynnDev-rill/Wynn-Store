...
class ImmortalApp : Application(), SingletonImageLoader.Factory {
    val repository by lazy { CatalogRepository(this) }
    ...
    override fun newImageLoader(context: PlatformContext) =
        ImageLoader.Builder(context)
            ...
            .diskCache {
                DiskCache.Builder()
                    .directory(File(cacheDir, "artwork").absolutePath.toPath())
...
class DataRefreshWorker(context: Context, parameters: WorkerParameters) :
    CoroutineWorker(context, parameters) {
    override suspend fun doWork(): Result {
        val repo = (applicationContext as ImmortalApp).repository
        repo.initialize()
        return if (repo.refresh()) Result.success()
