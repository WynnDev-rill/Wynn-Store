...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun refresh() {
        viewModelScope.launch { app.repository.refresh(true) }
    }

    fun rank(p: RankPosition) {
        viewModelScope.launch {
            app.preferences.setRank(
                Road.normalize(p, data.value.catalog?.rankRules ?: RankRules()),
                data.value.catalog?.season?.number,
            )
        }
    }

    fun theme(value: String) {
        viewModelScope.launch { app.preferences.setTheme(value) }
    }
